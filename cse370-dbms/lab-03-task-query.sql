...
Select first_name, last_name, employee_id, phone_number, salary, department_id from employees where (department_id, salary) in ( Select department_id, MIN(salary) from employees group by department_id) ORDER by department_id;
...
Select first_name, last_name, employee_id, commission_pct, department_id from employees where department_id = 7 and commission_pct <= ( Select MIN(commission_pct) from employees where department_id = 5);
...
SELECT department_id, COUNT(*) as Total_Count FROM employees GROUP BY department_id having MAX(salary) <= 30000;
...
SELECT department_id, job_id, commission_pct from employees where commission_pct < any ( Select MAX(commission_pct) from employees group BY department_id) ORDER BY commission_pct;

-- 6 ?
-- Find the manager_id who does not have any employee under them with a salary less than 3500.

SELECT manager_id FROM employees GROUP BY manager_id having MIN(salary) >= 3500;
